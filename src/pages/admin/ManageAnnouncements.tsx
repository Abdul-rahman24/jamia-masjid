      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {list.length === 0 ? (
          ...
          [...list].reverse().map(a => {
            ...
            return (
              <Card key={a.id} className={`overflow-hidden border-t-4 ${a.active ? 'border-t-green-500' : 'border-t-gray-300 opacity-60'}`}>
                {img && (
                ...
                <CardContent className="p-5">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${a.priority === 'Urgent' ? 'bg-red-100 text-red-700' : a.priority === 'Important' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}>
                      {a.priority}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                    ...
                    <button onClick={() => toggleActive(a.id, !a.active)} className={`text-sm font-bold px-3 py-1 rounded-full ${a.active ? 'bg-yellow-50 text-yellow-700 hover:bg-yellow-100' : 'bg-green-50 text-green-700 hover:bg-green-100'}`}>
